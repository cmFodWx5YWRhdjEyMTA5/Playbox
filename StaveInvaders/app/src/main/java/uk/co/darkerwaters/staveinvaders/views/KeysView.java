...
public class KeysView extends BaseView {
    ...
    public void setNoteRange(float minPitchDetected, float maxPitchDetected) {
        ...
        this.noteRange.setStart(notes.getChord(minPitchDetected));
        this.noteRange.setEnd(notes.getChord(maxPitchDetected));
        ...
        if (this.noteRange.getStart().equals(this.noteRange.getEnd())) {
            ...
            for (index = 0; index < notes.getSize(); ++index) {
                if (notes.getChord(index).equals(this.noteRange.getStart())) {
                    ...
                    int chordIndex = Math.max(index - 7, 0);
                    this.noteRange.setStart(notes.getChord(chordIndex));
    ...
    public void setNoteRange(Range newRange) {
        ...
        if (null != newRange && null != newRange.getStart() && null != newRange.getEnd()) {
            ...
            int startNoteIndex = notes.getChordIndex(this.noteRange.getStart().root().getFrequency());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (null != this.noteRange) {
            ...
            int iStart = singleChords.getChordIndex(this.noteRange.getStart().root().getFrequency());
            int iEnd = singleChords.getChordIndex(this.noteRange.getEnd().root().getFrequency());
            for (int i = iStart; i <= iEnd; ++i) {
                // for each chord in the range, find the letters highest and lowest
                Note note = singleChords.getChord(i).root();
                if (false == note.isFlat() && false == note.isSharp()) {
                    // this is a normal note
                    char primative = note.getNotePrimative();
                    if (primative <= lowestLetter) {
                        lowestLetter = primative;
                    }
                    if (primative >= highestLetter) {
                        highestLetter = primative;
                    }
                }
            }
